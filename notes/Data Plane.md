...
Questa è una cosa molto semplice, in pratica bisogna capire da una porta di ingresso quale sarà la porta d’uscita del router. Guardando l'intestazione del pacchetto. Se non fa match nessuna riga della tabella allora manda nel router di default che avrà un reach maggiore, più probabile che sappia dove mandare.

**Routing** invece va a scrivere mappe nel grafico della rete, cioè capisce quale sia il percorso più breve all'interno della rete, è parte del control plane.
...
in cui si va a guardare l'indirizzo di arrivo e si decide in questo modo come mandarti.
...
Ogni router ha una **flow table**, la stessa cosa di cui si parla nei IPv6 in [Livello di Rete#Datagramma IPv6](/notes/livello-di-rete#datagramma-ipv6) . Questa tabella è **costruita col routing plane centralizzato,** descritta dal software. Importante che tutta la rete sia SDN, sennò non funziona questo.
...
Questi switching fabrics lo fanno andare molto molto infretta
...
### Scheduling dei routers

Ci sono certi algoritmi di scheduling banali, come il First Come First served, e poi droppo tutti quelli che stanno fuori, oppure farli droppare a caso.
